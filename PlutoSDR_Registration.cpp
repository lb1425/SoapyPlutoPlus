	...
	for (std::vector<std::string>::iterator it = backends.begin(); it != backends.end(); it++) {
		...
		if (ret == 0) {
			...
			for (int i = 0; i < ret; i++) {
				...
				std::string description(iio_context_info_get_description(info[i]));

				// The description may contains an IP address that differentiate device with the same URI.
				auto sp_pos = description.find(" ");
				if (sp_pos != std::string::npos)
				{
					std::string addr = description.substr(0, sp_pos);
					unsigned char buf[sizeof(struct in6_addr)];
					if (inet_pton(AF_INET, addr.c_str(), buf) ||
							inet_pton(AF_INET6, addr.c_str(), buf))
					{
						// IPv4 or IPv6 regular format
						uri = "ip:" + addr;
					}
					else
					{
						...
						{
							// IPv6 link local format x:x:x:x:x%x
							std::string linkLocal = addr.substr(0, sep_pos);
							if (inet_pton(AF_INET6, linkLocal.c_str(), buf))
							{
								uri = "ip:" + addr;
							}
						}
